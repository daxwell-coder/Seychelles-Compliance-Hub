...
exports.onboardClientFunction = void 0;
...
const deterministicEncryption_1 = require("../../../functions/src/deterministicEncryption");
admin.initializeApp();
const db = admin.firestore();
// Simple inlined emitter (avoids separate module resolution issues in build environment)
let _pubsub = null;
function getPubSub() { if (!_pubsub)
    _pubsub = new pubsub_1.PubSub(); return _pubsub; }
async function emitEvent(type, payload = {}) {
    ...
    const enriched = { ...payload, type, emitted_at: new Date().toISOString(), event_id: (0, crypto_1.randomUUID)(), payload_hash };
    if (!topic) {
...
function sortKeys(obj) { if (Array.isArray(obj))
    return obj.map(sortKeys); if (obj && typeof obj === 'object') {
 ...
 */
const performSanctionsScreen = async (name) => {
...
exports.onboardClientFunction = (0, https_1.onRequest)(async (req, res) => {
    ...
    const executionId = req.id || (Date.now().toString(36) + Math.random().toString(36).slice(2));
    ...
    let overallScreeningStatus;
    try {
        const allNames = [
            validatedData.clientName,
            ...validatedData.beneficialOwners.map((bo) => bo.fullName),
        ];
        const settledResults = await Promise.allSettled(allNames.map(async (name) => ({ name, result: await performSanctionsScreen(name) })));
        ...
        screeningResults = settledResults
            ...
            .map((res) => res.value);
        const foundMatch = screeningResults.find((r) => r.result === "MATCH_FOUND");
        ...
        throw new https_1.HttpsError("unavailable", "The sanctions screening service is currently unavailable. Please try again later.");
    }
    ...
    try {
        ...
        const encryptedData = (0, deterministicEncryption_1.encryptOnboardingData)(validatedData);
        ...
        if (encryptedData.beneficialOwners.length < 499) {
            encryptedData.beneficialOwners.forEach((bo) => {
            ...
            throw new https_1.HttpsError("invalid-argument", "Exceeded maximum number of beneficial owners.");
        }
        await batch.commit();
